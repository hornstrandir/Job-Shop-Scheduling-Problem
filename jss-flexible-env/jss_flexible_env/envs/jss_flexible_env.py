...
class JssFlexibleEnv(gym.Env):
    def __init__(self, env_config=None):
        ...
        while line_str:
            ...
            if line_cnt == 1:
                ...
                self.nb_last_op_jobs = np.zeros(self.jobs, dtype=int)
                print(self.nb_last_op_jobs)
            else:
                # couple (machine, time)
                ...
                while i < len(split_data):
                    ...
                    self.nb_last_op_jobs[job_nb] = i // 2
            ...
            for job in range(self.jobs):
                if not self.legal_actions[job]:
                    if (
                        ...
                        and self.todo_time_step_job[job] + 1 <= self.nb_last_op_jobs[job]
                    ):
                        ...
                        while (
                            time_step < self.nb_last_op_jobs[job] and max_horizon > time_needed
                        ):
                            machine_needed = self.instance_matrix[job][time_step][0]
                            if (
                        ...
                        and self.todo_time_step_job[job] <= self.nb_last_op_jobs[job]
                    ):
                        time_step = self.todo_time_step_job[job]
                        machine_needed = self.instance_matrix[job][time_step][0]
                        time_needed = (
        ...
        for job in range(self.jobs):
            ...
            if was_left_time > 0:
                ...
                if self.time_until_finish_current_op_jobs[job] == 0:
                    ...
                    if self.todo_time_step_job[job] <= self.nb_last_op_jobs[job]:
                        ...
                        if self.legal_actions[job]:
                            self.legal_actions[job] = False
                            self.nb_legal_actions -= 1
            elif self.todo_time_step_job[job] <= self.nb_last_op_jobs[job]:
    ...
    def render(self, mode="human"):
        ...
        for job in range(self.jobs):
            i = 0
            while i <= len(self.instance_matrix[job]) and self.solution[job][i] != -1:
                dict_op = dict()
